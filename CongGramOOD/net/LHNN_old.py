...
class LHNN_old(nn.Module):
    def __init__(self, n_dim, c_dim, dim=32):
        super(LHNN_old, self).__init__()
        ...
        self.v6_readout = nn.Linear(dim, 2)
    ...
    def forward(self, v_n: torch.Tensor, v_c: torch.Tensor, g_nc: torch.Tensor, g_cn: torch.Tensor, na_cc: torch.Tensor
        ...
        v6_out = v6_out.permute(1,0)
        v6_out= v6_out.reshape(2,256,256)
        return v6_out, v3n_out

    @staticmethod
    def generate_adj(a_cc: torch.Tensor, h_nc: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        b1_nn = torch.diag(1/torch.sum(h_nc, dim=0))
        p1_cc = torch.diag(1/torch.sum(a_cc, dim=1))
        g_nc = h_nc
        g_cn = b1_nn @ h_nc.t()
        na_cc = p1_cc @ a_cc
