...
class GraphEmbed(nn.Module):
    def __init__(self, dim, ndim, embed_dim):
        super().__init__()
        self.conv1 = dglnn.HeteroGraphConv({
            'ccx1': dglnn.GraphConv(dim, embed_dim),
            'nn': dglnn.GraphConv(ndim, embed_dim)
        })
        self.conv2 = dglnn.HeteroGraphConv({
            'ccx1': dglnn.GraphConv(embed_dim, embed_dim),
            'nn': dglnn.GraphConv(embed_dim, embed_dim)
        })
    def forward(self, graph, h):
        x_src = {'Gcellx1': h['Gcellx1'], 'Gnet': h['Gnet']}
        x_dst = {'Gcellx1': h['Gcellx1'], 'Gnet': h['Gnet']}
        h = self.conv1(graph, (x_src, x_dst))
        h = {k:F.leaky_relu(v) for k, v in h.items()}
        x_src = {'Gcellx1': h['Gcellx1'], 'Gnet': h['Gnet']}
        x_dst = {'Gcellx1': h['Gcellx1'], 'Gnet': h['Gnet']}
        h = self.conv2(graph, (x_src, x_dst))
        return h
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.to_pixel = Rearrange('b (h w) c -> b c h w', h=pretrain_img_size)
        #pixel(seq) --> pixel 2D
        self.to_seq = Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = patch_size, p2 = patch_size, c=in_chans, h=pretrain_img_size // patch_size)
        #pixel 2D --> pixel(seq)
        self.g2px1 = Rearrange('(b n m) f -> b (n m) f', n=pretrain_img_size, m=pretrain_img_size)#batch_graph --> pixel(seq)
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=32, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        self.graph_embed = GraphEmbed(dim=in_chans, ndim=3, embed_dim=32)
...
class SwinUPer(nn.Module): 
    def __init__(self, chIn=3, chOut=2, chMid=512, img_size=256, patch_size=4, embed_dim=96, 
        ...
        self.decoder = UPerHead(in_channels=[embed_dim * 2**idx for idx in range(len(depths))], 
                                out_channels=embed_dim, out_shape=(img_size, img_size), 
