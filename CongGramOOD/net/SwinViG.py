...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        #     layer = gnn.HypergraphConv(basic_dim, basic_dim)
        #     self.hgnns.append(layer)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            print(x_out.shape)
            # splited = torch.chunk(x_out, chunks=x_out.shape[0])
            # feats = []
            # for j in range(len(splited)): 
            #     if len(h[i][j].shape) > 1 and len(c[i][j].shape) > 1: 
            #         feats.append(self.hgnns[i](splited[j], h[i][j], c[i][j]))
            #     else: 
            #         feats.append(splited[j])
            # x_out = torch.cat(feats, dim=0)
...
class SwinUPer(nn.Module): 
    def __init__(self, chIn=3, chOut=2, chMid=512, img_size=256, patch_size=4, embed_dim=96, 
        ...
        self.decoder = UPerHead(in_channels=[embed_dim * 2**idx for idx in range(len(depths))], 
                                out_channels=chOut, out_shape=(img_size, img_size), 
